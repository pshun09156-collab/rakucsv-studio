...
| JANコードの先頭ゼロが消える / 日付が勝手に変わる | 既存CSVをExcelで開いて編集している | 「CSVを読み込んで編集」でExcelを通さずに一括更新 |
...
### 3. 既存CSVを読み込んで編集する(v1.3)

RMSからダウンロードした `normal-item.csv` を読み込み、**価格・在庫をブラウザ上で一括更新**して書き戻せます。Excelを通さないので、UTF-8保存による全項目エラー・JANコードの先頭ゼロ落ち・日付の自動変換が起きません。

- 編集できる列: `通常購入販売価格` `在庫数` `倉庫指定` `サーチ表示` `カタログID`(商品管理番号・SKU管理番号・商品名は目印として表示)
- 一括操作: **価格を〇%変更** / **価格を〇円変更** / **在庫数を全行〇に設定**
- 商品管理番号・商品名で**絞り込んでから**一括操作できるため、全商品への誤爆が起きません
- 書き出しは読み込んだ全カラム・全行を保持し、**編集した値だけを差し替え**(触っていない列・行は1文字も変わりません)

### 4. データは外に出ません
すべての処理がブラウザ内で完結します。商品名・価格・在庫といった店舗の情報がサーバーに送信されることはありません。

## 既存商品の価格・在庫を一括更新する(v1.3)

RMSからダウンロードしたCSVを読み込み、絞り込んでから「価格を一括で〇%変更」「在庫を一括設定」などを適用し、書き出せます。**Excel経由で起きる事故(UTF-8保存による全項目エラー / JANコードの先頭ゼロ落ち / 日付の自動変換)が起きません。** 編集しなかった列・行は1文字も変更されずに出力されます。
...
手元のCSVを点検する場合は、ページ下部の「既存CSVの点検」にファイルを読み込ませてください。既存商品の価格・在庫をまとめて変更する場合は、その下の「CSVを読み込んで編集」をお使いください。
